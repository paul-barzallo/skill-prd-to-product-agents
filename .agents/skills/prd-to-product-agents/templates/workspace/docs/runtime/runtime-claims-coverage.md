...
| Enterprise proof is repeatable rather than rhetorical. | The publisher-maintained enterprise sandbox workflow and its uploaded evidence artifact, plus the manual runbook in `enterprise-readiness-sandbox.md`. | This is release evidence from the publishing repository, not a default workspace-local guarantee. |
| Execute tables define an intended role call set, not a hard runtime sandbox. | Prompt frontmatter, `validate ci prompt-tool-contracts`, `validate ci copilot-runtime-contract`, CODEOWNERS, PR governance, and immutable-governance review gates. | Runtime containment still depends on platform support and repository governance. Do not describe the table as arbitrary-shell enforcement. |
